...
1. Moduł generacji sieci: euklidesowych oraz losowych, o zadanej liczbie wierzchołków:
   ...
   - Podstawą komponentu będą moduły generowania sieci z pakietu _igraph_ - funkcje `Erdos_Renyi` (dla grafów losowych) oraz `GRG` (dla grafów euklidesowych).
...
Pierwszym etapem będzie przyjęcie zadanej liczby wierzchołków oraz zadanej gęstości grafu i obliczenie z nich docelowej liczby krawędzi $q_target$dla grafu wyjściowego. Drugi etap polegał będzie na wygenerowaniu grafu o n wierzchołkach połączonych losowo $q_{target}$ krawędziami.

Algorytm ten przyjmuje dwa argumenty: liczbę wierzchołków $n$ oraz współczynnik prawdopodobieństwa wystąpienia krawędzi $n$.

Grafy losowe ER (model Erdősa–Rényi) zostaną wygenerowane z użyciem funkcji `Erdos_Renyi` klasy `Grah` pakietu _igraph_. Metoda ta przyjmuje jako parametry:
...
Projektowane narzędzie wykorzystywać będzie implementacje grafów nieskierowanych z biblioteki _igraph_.
